...
for($i = 0; $i < count ( $CH ->ArrFormation ); $i ++) {
	...
	if($cl_sel_pages->buld_table[$CH ->ArrFormation[$i][0]]['ct_photo_id']) 
		$img = "<img src=\"../../files/images/shops/sm_{$cl_sel_pages->buld_table[$CH ->ArrFormation[$i][0]]['ct_photo_id']}.{$cl_sel_pages->buld_table[$CH ->ArrFormation[$i][0]]['ct_photo_file_type']}\" width=\"100\"/>";
	...
	$pagesReturn .= "<td>{$cl_sel_pages->buld_table[$CH ->ArrFormation[$i][0]]['brand_name']}</td>";
	$pagesReturn .= "<td>{$cl_sel_pages->buld_table[$CH ->ArrFormation[$i][0]]['dict_name']}</td>";
