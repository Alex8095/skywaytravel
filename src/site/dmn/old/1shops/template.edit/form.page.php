...
<div id='errOutput' class="errOutput"></div>
	...
	<fieldset>
		...
		<select name="brand_id" class="zpFormRequired">
			<?php echo sel_parent_id ( $brandsParent,  $active_id ['brand_id'], 'ct_id', 'ct_name' ); ?>
		</select>
		<br />
		<label class='zpFormLabel'>Город</label> 
		<select name="city_id" class="zpFormRequired">
			<?php echo sel_parent_id ( $cityDict, $active_id ['city_id'], 'dict_id', 'dict_name' ); ?>
		</select>
		<br />
		<label class="zpFormLabel">Позиция</label>
		<input value="<?php echo $active_id ['pos'];?>" name="pos" type="text" class="zpForm zpFormInt" /> 
		<br />	
