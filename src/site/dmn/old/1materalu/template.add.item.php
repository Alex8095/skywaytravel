...
$cl_sel_pages->select_table_query ( "select c.*, ci.ct_photo_id, ci.is_main from {$tbl_catalog} c
									 left join {$tbl_ct_photos} ci on ci.ct_id = c.ct_id and ci.is_main = 1
									 WHERE c.lang_id = {$lang_id} and c.dict_id = '4cf62997873ef' and ci.lang_id = {$lang_id} ORDER BY c.pos desc LIMIT 1" );
