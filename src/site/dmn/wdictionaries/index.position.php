...
<ul>
	<li><label>каталог справочников</label><br />
	<select  class="d-filter-select"  name="position_id">
		<option value="">не выбрано</option>
                        <?php
	echo printFormSelection ( $cl_sel_pages->table, $_GET ['position_id'] );
	?>
                    </select></li>
</ul>
<input type="submit" class="d-filter-button-search ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" value="Применить"></form>
</div>

<form id="myForm" action="" method="post"><input
	value="<?php
	echo $_GET [position_id];
	?>" size="13" name="position_id"
	type="hidden">
<div class="eventForm">
<a href="#" class="ui-state-default ui-corner-all bottom-padding" title="добавить значение" id="submitAdd"><span class="ui-icon ui-icon-plus"></span>добавить значение</a> <a href="#" class="ui-state-default ui-corner-all bottom-padding" title="редактировать значение" id="submitEdit"><span class="ui-icon ui-icon-pencil"></span>редактировать значение</a> <a href="#" class="ui-state-default ui-corner-all bottom-padding" title="удалить значение" id="submitDell"><span class="ui-icon ui-icon-trash"></span>удалить значение</a>
    <div class="clean"> </div>
</div>
<!--	-->
...
$(document).ready(function(){
	...
	$("#outputWindows").dialog({
		...
		$("#loading").ajaxStart(function(){
  		$(this).show();});
		$('#DivRequest').load('template.load.php?print=position_portfolio&position_id=<?php
	echo $_GET [position_id];
	?>');
...
function showWindowsAdd() {
	$('#outputWindows').dialog({ title: 'Добавление значение справочника' });
	$('#outputWindows').dialog('open');
	return true;
}
function showWindowsEdit() {
	$('#outputWindows').dialog({ title: 'Редактирование значение справочника' });
	$('#outputWindows').dialog('open');
	return true;
}
...
function valideEdit(formData, jqForm, options)
{
	...
	if(queryString.search("dict_id") == 0) 
	{
