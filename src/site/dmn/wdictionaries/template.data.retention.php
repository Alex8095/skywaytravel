...
if ($_POST ['retention'] == 'add_page') {
	$_POST ['ld_code'] = substr ( mysql_real_escape_string ( $_POST ['ld_code'] ? $_POST ['ld_code'] : translit ( $_POST [ld_name] ) ), 0, 12 );
...
/*
 * $response['fieldErrors']['title'] = "Passwords do not match"; if(sizeof($response['fieldErrors']) == 0) { $response['success'] = true; header('Content-type: text/plain'); echo $json_converter->encode($response); } else { header('Content-type: text/plain'); echo $json_converter->encode($response); }
 */
?>
