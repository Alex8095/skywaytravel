...
if ($_GET ['print'] == 'print_ptm') {
	...
	$ClPTMCel->select_table_query ( "SELECT p.* FROM {$tbl_sturture} p WHERE p.lang_id = {$_COOKIE[lang_id]} order by p.pos", 'page_id' );
	
	$PSSortObj = new catalogClass($ClPTMCel->table, $ClPTMCel->buld_table, 'p_menu', 'page_id', 'parent_id');
	$PSSortObj -> get_arr_formation();
	
//	echo "<prE>";
//	print_r($PSSortObj->ArrFormation);
//	echo "</prE>";
