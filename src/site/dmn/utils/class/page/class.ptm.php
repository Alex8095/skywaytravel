...
class ptm {
    ...
    * @return 
    */
	private function processWebTKD($fileTemplate, $body = array(), $p_w_title = NULL, $p_w_keyw = NULL, $p_w_desc = NULL) {
		return Controller::Template ( $fileTemplate, array_merge ( ($body ? $body : array()), array ("p_w_title" => $this->buildMetaTitleField ( $p_w_title ), "p_w_keyw" => ($p_w_keyw ? $p_w_keyw : $this->data_ptm ['active_page_data'] ['p_w_keyw']), "p_w_desc" => $this->buildMetaDescriptionField ( $p_w_desc ), "BrowserVersion" => $this->browser_detection (), "cookie_lang_code" => $_COOKIE ['lang_code'], "PreSrc" => $this->conf ['PreSrc'], "php_self" => $this->data_ptm ['active_page_data'] ['page_url'], "arWords" => $this->arWords, 'data_ptm' => $this->data_ptm ) ) );
	}
