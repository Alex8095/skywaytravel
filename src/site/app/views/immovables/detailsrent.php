			...
			<div class="colomns">
				<div class="colomn price-prop-colomn" >
					<div class="price-prop">
                		...
                		<p class="price-eur">&#8364; <?php echo Discharge::GetDisValue (($Model->item ["im_prace_manth"] * $exchangeRateObj->GetItemData('USD'))/$exchangeRateObj->GetItemData('EUR'), 4, "");?></p>
						<p class="price-rub"><span>ք</span> <?php echo Discharge::GetDisValue (($Model->item ["im_prace_manth"] * $exchangeRateObj->GetItemData('USD'))/$exchangeRateObj->GetItemData('RUB'), 4, "");?></p>
						<?php if($Model->item ["im_prace_day"]): ?>
							<p class="price-msq"><?php echo Discharge::GetDisValue ( $Model->item ["im_prace_day"] * $exchangeRateObj->GetItemData('USD'), 4 );?> / $ <?php echo Discharge::GetDisValue ($Model->item ["im_prace_day"], 4, "");?> сутки</p>
						<?php endif;?>
						...
						<?php if($area): ?>
							<a href="/<?php echo $arWords["TypeCatImNameArrIdPAge"][$Model->item["im_catalog_id"]];?>/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>?<?php echo  $m->urlDictToParent($Model->item["im_area_id"])?>&action=ImFormSearch" title="<?php echo $area;?> район" class="city-area"><?php echo $area;?> район</a>
						<?php elseif ($city): ?>	
							<a href="/<?php echo $arWords["TypeCatImNameArrIdPAge"][$Model->item["im_catalog_id"]];?>/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>?<?php echo  $m->urlDictToParent($Model->item["im_city_id"], 2)?>&action=ImFormSearch" title="<?php echo $city;?>" class="city-area"><?php echo $city;?></a>
						<?php endif;?>
						<div class="clear"></div>
						...
						<div class="report-inner">
							<a class="send-to-friend" href="javascript:SendImToFriend(<?php echo $Model->item["im_id"]?>);" alt="<?php echo getLangString("SubmitImFriend");?>" title="<?php echo getLangString("SubmitImFriend");?>"><span class="bg-icons">&nbsp;</span></a>
...
<?php
	$paramSimilar = array("is_cashe" => true, "im_is_rent" => true, "hide" => "show",  "im_catalog_id" => $Model->item["im_catalog_id"], "im_id" => $Model->item["im_id"], "im_area_id" => $Model->item["im_area_id"], "im_adress_id" => $Model->item["im_adress_id"], "im_space_like" => $Model->item["im_space"], "im_prace_like" => $Model->item["im_prace"], "limit" => 10);
