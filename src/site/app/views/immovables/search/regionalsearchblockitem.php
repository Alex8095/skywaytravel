<div class="rlist rListItem-<?php echo $margin; ?> parent-element-<?php echo $parent_id;?>" style="margin:0 0 0 <?php echo ($margin ? $margin : 0)*10; ?>px; <?php echo (($parent_id == 0 ? "false" : $is_checked) ? "" : "display:none;"); ?>">
	<?php foreach ($list as $key => $value) :?>
		<div class="item i-<?php echo $value["0"]?>">
			<?php if(is_array($value["child"])):?>
				<span class="plus " id="plus-item-<?php echo $value["0"]?>"><?php echo ($value["checked"] ? '-' : "+")?></span>
			<?php endif;?>
			<div class="val"><input class="checkbox-item-<?php echo $value["0"]?>" <?php echo ($value["checked"] ? 'checked="checked"' : "")?> accesskey="" value="1" name="<?php echo $value["dict_id"]?>_<?php echo $value["2"]?>" id="id_<?php echo $value["dict_id"]?>_<?php echo $value["2"]?>" onchange=""  type="checkbox"/><label for="id_<?php echo $value["dict_id"]?>_<?php echo $value["2"]?>" class="r-name-item-<?php echo $value["0"]?>"><?php echo $value["dict_name"];?></label></div>
			<div class="clear"></div>
			<?php 
				if($value["child"]) {
