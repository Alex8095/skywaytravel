<?php if($Model->typeVakansiiList):?>
	...
	<?php foreach ($Model->typeVakansiiList as $key => $value):?>
		<li>
			<a class="" href="/vakansii/index/<?php echo $value['dict_id']?>" title="<?php echo $value['dict_name']?>"><?php echo $value['dict_name']?></a>
			...
			<?php if($Model->list):?>
				<ul class="sitemap-ul inner-<?php echo $level + 1;?>">
					<?php foreach ($Model->list as $ikey => $ivalue):?>
						<li>
							<a class="" href="/vakansii/item/<?php echo $ivalue['url']?>" title="<?php echo $ivalue['w_title']?>"><?php echo $ivalue['title']?></a>
						</li>
					<?php endforeach;?>
				</ul>
			<?php endif;?>
