...
<?php //devLogs::_printr($Model->dictionaries);?>
...
// 				$area = $city;
		...
		<div class="item colomns i-<?php echo $value["im_id"]; ?>">
			<div class="colomn w-200 img">
				...
				<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>" title="<?php echo $im_title;?>"><img src="<?php echo getLangString('imageDomain');?>/files/images/immovables/si_<?php echo $value["im_photo"];?>" width="180" alt="<?php echo $im_title;?>"/></a>
				<div class="colomns">
					<?php if($value["photos"]): ?>
			...
			<?php if($value["im_adress_id"]): ?><h3 class="street"><a href="/<?php echo $Model->getitemlink($value);?>" title="<?php echo $im_title;?>"><?php echo ($value["im_adress_id"] ? $Model->dictionaries->getItemValue($value["im_adress_id"]) : $Model->dictionaries->getItemValue($value["im_city_id"]))?><?php echo ($value["im_adress_house"] ? sprintf(", %s", $value["im_adress_house"]) : ""); ?></a></h3><?php endif;?>
				<div class="colomns ">
					<div class="colomn w-160">
						<?php if($area): ?>
							<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>?<?php echo  $m->urlDictToParent($value["im_area_id"])?>&action=ImFormSearch" title="<?php echo $area;?> район" class="city-area"><?php echo $area;?> район</a>
						<?php elseif ($city): ?>	
							<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>?<?php echo  $m->urlDictToParent($value["im_city_id"], 2)?>&action=ImFormSearch" title="<?php echo $city;?>" class="city-area"><?php echo $city;?></a>
						<?php endif;?>
						<div class="clear"></div>
						<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>" title="<?php echo $im_title;?>"  class="im-code"><?php echo getLangString('ImFListHeaderCodeN');?>: <?php echo $value["im_code"];?></a>
					</div>
				...
				<div class="buttoms">
					<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>" title="<?php echo $im_title;?>" class="readmore"><span class="bg-icons">&nbsp;</span><?php echo getLangString('imReadMore');?></a>
                    <a target="_blank" href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>" title="<?php echo $im_title;?>" class="readmoreblank"><span class="bg-icons">&nbsp;</span><?php echo getLangString('imReadMoreBlank');?></a>
					<a href="/commercial/rent/<?php echo $routingObj->getParamItem("page_id", 1);?>/<?php echo $value["im_id"]; ?>#map" title="<?php echo $im_title;?>" class="map"><span class="bg-icons">&nbsp;</span><?php echo getLangString('imReadMoreMap');?></a>
