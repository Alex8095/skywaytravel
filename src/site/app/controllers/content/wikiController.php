...
class wikiController extends aControllerClass {
	public function index($param) {
		$this->redirect ( "wiki/item/Бытовая техника" );
	...
	public function item($param) {
		...
		if ($model->item) {
			...
			$this->appDataObj->setStringNavigationLast ( $model->item ["w_menu_name"] );
		...
		if ($param ["action"] == "index") {
			...
			if ($model->list) {
				foreach ( $model->list as $key => $value ) {
					$array [] = array (
							"loc" => sprintf ( "http://%s/wiki/item/%s", $_SERVER ['HTTP_HOST'], $value ["w_menu_name"] ),
