...
class vakansiiController extends aControllerClass {
	public function index($param) {
		...
		if (empty ( $param["type_id"] ))
			$this->redirect ( sprintf ( "vakansii/index/%s", $model->typeVakansiiList[0]["dict_id"] ) );
		$param["hide"] = true;
		$model->getList ( $param );
		$this->appDataObj->setStringNavigationLast ( $model->typeVakansiiList[0]["dict_name"] );
	...
	public function item($param) {
		...
		if ($model->item) {
			...
			$this->appDataObj->setStringNavigation ( sprintf ( ' <a href="/vakansii/index/%s" title="">%s</a><span class="next">~>~</span><span class="last">%s</span>', $model->item["type_id"], $model->dictionaries->buld_table[$model->item["type_id"]]["dict_name"], $model->item["title"], $model->item["title"] ) );
			$this->appDataObj->setPAction ( "index" );
		}
		return $this->View ( array(
				"Model" => $model) );
		...
		if ($param["action"] == "index") {
			...
			if ($model->typeVakansiiList) {
				foreach ( $model->typeVakansiiList as $key => $value ) {
					$array[] = array(
							"loc" => sprintf ( "http://%s/vakansii/index/%s", $_SERVER['HTTP_HOST'], $value["dict_id"] ),
					...
					if ($model->list) {
						foreach ( $model->list as $ikey => $ivalue ) {
							$array[] = array(
									"loc" => sprintf ( "http://%s/vakansii/item/%s", $_SERVER['HTTP_HOST'], $ivalue["url"] ),
									"lastmod" => date ( "Y-m-d H:i:s" ),
									"changefreq" => "weekly",
									"priority" => $param["priority"]);
						}
