...
class landController extends aControllerClass {
	...
	public function rent($param) {
		...
		$model->getListPager ( $param, $param["page_id"], "/land/rent" );
		$this->buildImmovablesHttpHead ( $param, $model, "land", "rent" );
	...
	public function sale($param) {
		...
		$param["hide"] = "show";
		$model = new immovablesModelClass ( new immovablesProviderClass ( "immovables", "im_id" ) );
		$model->getListPager ( $param, $param["page_id"], "/land/sale" );
		$this->buildImmovablesHttpHead ( $param, $model, "land", "sale" );
		return $this->View ( array(
				"Model" => $model) );
	}
		...
		if ($param["action"] == "rent" || $param["action"] == "sale") {
			...
			if ($model->list) {
				foreach ( $model->list as $key => $value ) {
					$array[] = array(
							"loc" => sprintf ( "http://%s/home/%s/1/%s", $_SERVER['HTTP_HOST'], $param["action"], $value["im_id"] ),
