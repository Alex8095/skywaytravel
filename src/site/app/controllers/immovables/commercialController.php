...
class commercialController extends aControllerClass {
	...
	public function rent($param) {
		...
		$model->getListPager ( $param, $param["page_id"], "/commercial/rent" );
		$this->buildImmovablesHttpHead ( $param, $model, "commercial", "rent" );
	...
	public function sale($param) {
		...
		$param["hide"] = "show";
		$model = new immovablesModelClass ( new immovablesProviderClass ( "immovables", "im_id" ) );
		$model->getListPager ( $param, $param["page_id"], "/commercial/sale" );
		$this->buildImmovablesHttpHead ( $param, $model, "commercial", "sale" );
		return $this->View ( array(
				"Model" => $model) );
	}
		...
		if ($param["action"] == "rent" || $param["action"] == "sale") {
			...
			if ($model->list) {
				foreach ( $model->list as $key => $value ) {
					$array[] = array(
							"loc" => sprintf ( "http://%s/commercial/%s/1/%s", $_SERVER['HTTP_HOST'], $param["action"], $value["im_id"] ),
