...
class flatController extends aControllerClass {
	...
	public function rent($param) {
		...
		$model->getListPager ( $param, $param["page_id"], "/flat/rent" );
		$this->buildImmovablesHttpHead ( $param, $model, "flat", "rent" );
// 		devLogs::_printr($model->list);
	...
	public function sale($param) {
		...
		$param["hide"] = "show";
		$model = new immovablesModelClass ( new immovablesProviderClass ( "immovables", "im_id" ) );
		$model->getListPager ( $param, $param["page_id"], "/flat/sale" );
		$this->buildImmovablesHttpHead ( $param, $model, "flat", "sale" );
		return $this->View ( array(
				"Model" => $model) );
	}
	/* sitemap */
		...
		if ($param["action"] == "rent" || $param["action"] == "sale") {
			...
			if ($model->list) {
				foreach ( $model->list as $key => $value ) {
					$array[] = array(
							"loc" => sprintf ( "http://%s/flat/%s/1/%s", $_SERVER['HTTP_HOST'], $param["action"], $value["im_id"] ),
