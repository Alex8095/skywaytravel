<?php
class serviceProviderClass extends providerClass {
	...
	public function getItemParam($param) {
		...
		$res = $this->mysql->select_table_id ( "WHERE lang_id = {$_COOKIE['lang_id']} {$query} ORDER BY pos desc" );
		return $res;
	}
	public function getList($param) {
		...
		$this->mysql->select_table_query ( "select p.* from {$this->table} p
										    WHERE p.lang_id = {$_COOKIE['lang_id']} {$query} ORDER BY pos desc", $this->id );
