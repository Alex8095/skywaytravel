...
class immovablesModelClass extends modelClass {
	...
	public $propertiesGroupList;
	public $propertiesGroupListGroup;
	/* for im. item */
	public $imagesList;
	...
	public function getListPager($param, $page_id, $pagerlink, $limit = null, $providerMethod = "getListPager") {
		...
		$orderby = "im_date_add";
		if ($_COOKIE["im_where_sort"] != "im_space")
			$orderby = (substr ( $pagerlink, strlen ( $pagerlink ) - 4, strlen ( $pagerlink ) ) == "sale" ? "im_prace" : "im_prace_manth");
		else
			$orderby = $_COOKIE["im_where_sort"];
		$this->provider->setValue ( "orderby", $orderby );
		...
		$propParam = array(
				"hide" => "show");
		if ($param["im_catalog_id"])
			$propParam = $param["im_catalog_id"];
		...
		if ($providerMethod == "getListPager")
			$this->provider->getListPager ( $param, $page_id, $query );
		if ($providerMethod == "getListPagerMap")
			$this->provider->getListPagerMap ( $param, $page_id, $query );
	...
	public function getPropertiesOnlyGroupList($param) {
		$res = $this->provider->getPropertiesOnlyGroupList ( $param );
		$this->propertiesListGroup = $res["list"];
		$this->propertiesListGroupBuild = $res["listBuild"];
	}
	...
	public function getItemNoHide($id) {
		$this->item = $this->provider->getItemNoHide ( $id );
	}
	...
	public function getitemlink($item = "") {
		if (empty ( $item ))
			$item = $this->item;
		global $arWords;
		return sprintf ( "%s/%s/1/%s", $arWords["TypeCatImNameArrIdPAge"][$item["im_catalog_id"]], ($item["im_is_sale"] ? "sale" : "rent"), $item["im_id"] );
	}
	public function getMayByList($param, $page_id, $pagerlink) {
		...
		global $routingObj;
		$paramformayby["hide"] = "show";
		if ($routingObj->getAction () == "rent")
			$paramformayby["im_is_rent"] = 1;
		if ($routingObj->getAction () == "sale")
			$paramformayby["im_is_sale"] = 1;
	...
	public function saveItem($param) {
		$resultProvider = $this->provider->saveItem ( $param );
		$this->item["im_id"] = $resultProvider["id"];
		return $this->item;
	}
	public function saveItemProperties($param) {
		$this->provider->saveItemProperties ( $param );
		$this->getItemNoHide ( $param["im_id"] );
		return $this->item;
	}
	public function saveItemImages($param, $files) {
		devLogs::_printr ( $param );
		devLogs::_printr ( $files );
		if ($files) {
			foreach ( $files as $key => $value ) {
				$fileName = uniqid ();
				$extension = pathinfo ( $files[$key]['name'] );
				$extension = strtolower ( $extension['extension'] );
				echo $fileName = strtolower ( $fileName . "." . $extension );
			}
		}
		return true;
	}
	public function saveItemSummary($param) {
		$this->provider->saveItemSummary ( $param );
		$this->getItemNoHide ( $param["im_id"] );
		return $this->item;
	}
}