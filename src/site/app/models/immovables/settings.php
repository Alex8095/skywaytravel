<?php
$_COOKIE ['im_f_show_style'] = ($_COOKIE ['im_f_show_style'] ? $_COOKIE ['im_f_show_style'] : "im_list_table");
$_COOKIE ['im_f_show_pnumber'] = ($_COOKIE ['im_f_show_pnumber'] ? $_COOKIE ['im_f_show_pnumber'] : "30");
$_COOKIE ['im_where_sort'] = ($_COOKIE ['im_where_sort'] ? $_COOKIE ['im_where_sort'] : "im_prace");
$_COOKIE ['im_where_sort_order'] = ($_COOKIE ['im_where_sort_order'] ? $_COOKIE ['im_where_sort_order'] : "desc");

$DefaultFormValue ['im_space'] = array (
		1,
		100000 
);
$DefaultFormValue ['im_prace'] = array (
		1,
		100000000 
);
$DefaultFormValue ['im_prace_sq'] = array (
		1,
		100000 
);
$DefaultFormValue ['im_prace_manth'] = array (
		1,
		100000 
);
$DefaultFormValue ['im_prace_day'] = array (
		1,
		10000 
);

$maybymatrix = array (
		array (
				"maybyDeleteParamFieldsMyltiCheckbox" 
		),
		array (
				"maybyDeleteParamFieldsCheckbox" 
		),
		array (
				"maybyDeleteParamFieldsSlider" 
		),
		array (
				"maybyDeleteParamFieldsTerritory" 
		),
		array (
				"maybyDeleteParamFieldsCheckbox",
				"maybyDeleteParamFieldsMyltiCheckbox",
				"maybyDeleteParamFieldsIm" 
		),
		array (
				"maybyDeleteParamFieldsIm",
				"maybyDeleteParamFieldsSlider" 
		),
		array (
				"maybyDeleteParamFieldsCheckbox",
				"maybyDeleteParamFieldsMyltiCheckbox",
				"maybyDeleteParamFieldsIm" 
		),
		array (
				"maybyDeleteParamFieldsCheckbox",
				"maybyDeleteParamFieldsMyltiCheckbox",
				"maybyDeleteParamFieldsIm",
				"maybyDeleteParamFieldsIm",
				"maybyDeleteParamFieldsSlider" 
		) 
);
global $maybymatrix;
/*
maybyDeleteParamFieldsCheckbox
maybyDeleteParamFieldsMyltiCheckbox
maybyDeleteParamFieldsIm
maybyDeleteParamFieldsTerritory
maybyDeleteParamFieldsSlider
*/