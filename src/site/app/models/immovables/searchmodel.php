...
class immovablesSearchModelClass extends immovablesModelClass {
	...
	public function buildImmovablesProperties($categoryType = "") {
		...
		$convertParamForProperties = $this->convertParamForProperties ( $this->routingObj->getParam () );
		// ыборка характеристик недвижимости
		$ImPropListSt = new mysql_select ( "im_properties_list", "WHERE lang_id = {$_COOKIE['lang_id']} " . ($categoryType ? " AND catalog_id='{$categoryType}'" : "") . " AND type_prop='standard' AND hide='show' AND is_show_form=1 $query", "ORDER BY im_prop_style_id ASC" );
		$ImPropListSt->select_table ( "im_prop_id" );
		$PrintPropFormSt = new ImPropAdvaced ( $ImPropListSt, $this->dictionaries, $convertParamForProperties, NULL, false, "SearchFormAdvased", "FormSearchInputText", getLangString ( 'FormSearchFirldNoValue' ) );
		$PrintPropFormSt->ImPropListPrintField ();
		$SearchListPropCat = $PrintPropFormSt->Form;
		$this->PrintPropFormSt = $PrintPropFormSt;
		
		// ыборка характеристик недвижимости
		$ImPropListAd = new mysql_select ( "im_properties_list", "WHERE lang_id = {$_COOKIE['lang_id']} " . ($categoryType ? " AND catalog_id='{$categoryType}'" : "") . " AND type_prop='advanced' AND hide='show' AND is_show_form=1 {$query}", "ORDER BY im_prop_style_id ASC" );
		$ImPropListAd->select_table ( "im_prop_id" );
		$PrintPropFormAd = new ImPropAdvaced ( $ImPropListAd, $this->dictionaries, $convertParamForProperties, NULL, false, "SearchFormAdvased", "FormSearchInputText", getLangString ( 'FormSearchFirldNoValue' ) );
		$PrintPropFormAd->ImPropListPrintField ();
		$this->PrintPropFormAd = $PrintPropFormAd;
// 		devLogs::_printr($PrintPropFormAd);
		// ыборка характеристик недвижимости
		$ImPropList = new mysql_select ( "im_properties_list", "WHERE lang_id = {$_COOKIE['lang_id']} " . ($categoryType ? " AND catalog_id='{$categoryType}'" : "") . " AND hide='show'", "ORDER BY im_prop_style_id ASC" );
		$ImPropList->select_table ( "im_prop_id" );
		$PrintPropForm = new ImPropAdvaced ( $ImPropList, $this->dictionaries, $convertParamForProperties, NULL, false, "SearchFormAdvased", "FormSearchInputText", getLangString ( 'FormSearchFirldNoValue' ) );
		$PrintPropForm->ImPropListPrintField ();
		$this->PrintPropForm = $PrintPropForm;
	}
	private function convertParamForProperties($array) {
		$ret = array();
		if (! empty ( $array ))
			foreach ( $array as $key => $value ) {
				$ret[$key] = $value;
				if (substr ( $key, 0, 2 ) == "m_")
					$ret[substr ( $key, 0, 15 )][] = $value;
			}
		return $ret;
	}
}