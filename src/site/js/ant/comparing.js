...
$(document).ready(function() { 
	...
	$(".comparing-objects-link").click(function () {
		...
		systemalert("Объект добавлен в отобранные!");
		$(this).hide();
		return false;
	});
...
function comparisonAddItem(im_id) {
	$.ajax({
        ...
        url: "/immovables/comparingadditem?im_id=" + im_id
	});
}
function comparisonRemoveItem(im_id) {
	$.ajax({
        ...
        url: "/immovables/comparingremoveitem?im_id=" + im_id
	});
}
/**
...
function comparisonSetSorted(array) {
	...
	$.ajax({
        ...
        url: "/immovables/comparingsetsorted?list=" + list
	});
}
/**
...
function comparisonHideEmptyAndLargeParam() {
	...
	comparisonParamBackground();
}
/**
 * закрашиваем фон через 1
 */
var comparisonParamBackgroundI = 0;
function comparisonParamBackground() {
	$( ".param-list span, .param-list a" ).each(function() {
		var param = $(this).attr("class");	
		if( (comparisonParamBackgroundI % 2 ) == 1) {
			$("." + param).css("background", "#dfdfdf");
		}
		comparisonParamBackgroundI++;
	});
	comparisonParamBackgroundI--;
	$( ".param-list .properties span, .param-list .properties a" ).each(function() {
		var param = $(this).attr("class");	
		if( (comparisonParamBackgroundI % 2 ) == 1) {
			param = param.replace(/span/g, "");
			$("." + param).css("background", "#dfdfdf");
		}
		comparisonParamBackgroundI++;
	});
