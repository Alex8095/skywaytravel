...
function SendImToFriendIsTrue(v,m,f){
	...
	//var Ajaxload = "/report/sendtofriend?im_id="+Fim_id+"&name="+FFriendName+"&email="+FemailFriend; 
...
function setStylePnumber(Fid,FValue) {
	$('#DivRequest').load('/immovables/setCookie?key='+Fid+'&value='+FValue+'');
...
function setSortTable(FValue) {
	$('#DivRequest').load('/immovables/setsorttable?key='+FValue+'');
	//alert('asd');
	$("#DivRequest").ajaxComplete(function(){
  		//location.href = location.href.toLowerCase();
		location.href = location.href;
