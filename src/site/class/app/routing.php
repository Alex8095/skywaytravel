...
class routingClass {
	...
	public function getControllerFull() {
		return sprintf ( "%sController", $this->controller );
	}
	...
	public function init() {
		...
		if (! empty ( $get[1] ))
			$this->controller = strtolower ( $get[1] );
		if (! empty ( $get[2] ))
			$this->action = strtolower ( $get[2] );
		$this->buildGet ( $get );
		// $this->buildPost ( $get );
	}
	/*
	 * public function init() { //преобразование строки REDIRECT_URL в массив $get = explode ( '/', $_SERVER ['REDIRECT_URL'] ); if (! empty ( $get [1] )) $this->localization = strtolower ( $get [1] ); if (! empty ( $get [2] )) $this->controller = strtolower ( $get [2] ); if (! empty ( $get [3] )) $this->action = strtolower ( $get [3] ); $this->buildGet ( $get ); //$this->buildPost ( $get ); }
	 */
	private function buildGet($strRedirectArray) {
		if (isset ( $this->routes[$this->controller][$this->action] )) {
			...
			foreach ( $strRedirectArray as $key => $value ) {
				if (isset ( $key, $this->routes[$this->controller][$this->action][$key] ))
					$this->get[$this->routes[$this->controller][$this->action][$key]] = $value;
			}
		}
		//
