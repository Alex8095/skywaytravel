...
class ModuleSiteIm {
	...
	//
	/*
	 * Функция: обрабатывает массив и подставляет данные в массив @param $IdTemplateHtml - айди шаблона @param $ArrRequest - выбранный массив @param $ArrIdAndAction -	массив айдишников данных, и функция обработчиков
	 */
	...
	/*
	 * Функция: возвращает значение справочника по айди @param $data	- @param $key		-
	 */
	...
	/*
	 * Функция: возвращает другую стоимость обьекта, если аренда то продажу, и наоборот @param $data	- @param $key		-
	 */
	...
	function GetAdressTable($data, $key) {
		$AdrArr = array(
				'im_region_id' => 'FormSearchNameRegion',
				'im_a_region_id' => 'FormSearchNameRRegionN',
				'im_city_id' => 'FormSearchNameCity',
				'im_area_id' => 'FormSearchNameACityN',
	...
	function GetStandartTable($data, $key) {
		$AdrArr = array(
				'im_code' => 'ImFListHeaderCodeN',
				'im_prace_sq' => 'FormSearchNamePriceMS',
				'im_prace_day' => 'FormSearchNamePriceDay',
				'im_prace_manth' => 'FormSearchNamePriceManth',
				'im_space' => 'FormSearchNameSq',
				'4c4069e4f04ec' => 'FormSearchNameSqYchastka');
	...
	function GetFullAdress($data, $key) {
		...
		// m_city_id#, #im_adress_id#,========== #im_adress_house#, #im_adress_flat#
	...
	function GetCurrentPriceOne($data, $key) {
		return Controller::Template ( "application/template/immovables/price.current.item.php", array(
				"data" => $data,
				"key" => $key) );
	}
	function getExchangeRateBlock($value) {
		return Controller::Template ( "application/template/exchangeRate/exchange.rate.block.php", array(
				"value" => $value) );
	}
	function GetCurrentPrice($data, $key) {
		return appHtmlClass::partial ( "immovables/price/pricecurrent", array(
				"Data" => $data,
				"key" => $key) );
	}
	function GetCurrentPriceNoImage($data, $key) {
		return appHtmlClass::partial ( "immovables/price/pricecurrentnoimage", array(
				"Data" => $data,
				"key" => $key) );
	}
	...
	function GetSimplePrice($data, $key) {
		...
		if (empty ( $data[$key] ))
			...
			return appHtmlClass::partial ( "immovables/price/pricesimple", array(
					"Data" => $data,
					"key" => $key) );
	}
	...
	function GetPropValueMini($imitem, $im_prop_id, $im_prop_style_id) {
		$ret = "";
		switch ($im_prop_style_id) {
			case "4c3ec1f67fe1b" : // Выпадающий список
				{
					$dict_id = $this->ImPropArrData[$imitem['im_id']][$im_prop_id]['im_prop_value_dict_id'];
					$ret = (empty ( $dict_id ) ? $ret : $this->DictData->getItemValue ( $dict_id ));
					break;
				}
			case "4c3ec1f67fe12" : // radio
				{
					$dict_id = $this->ImPropArrData[$imitem['im_id']][$im_prop_id]['im_prop_value_dict_id'];
					$ret = (empty ( $dict_id ) ? $ret : $this->DictData->getItemValue ( $dict_id ));
					break;
				}
			case "4c3ec1f67fe11" : // checkbox
				{
					$val = $this->ImPropArrData[$imitem['im_id']][$im_prop_id]['im_prop_value'];
					$ret = (empty ( $val ) ? $ret : "+");
					break;
				}
			case "4c3ec3ad35af9" : // text
				{
					$val = $this->ImPropArrData[$imitem['im_id']][$im_prop_id]['im_prop_value'];
					$ret = (empty ( $val ) ? $ret : $val);
					break;
				}
			case "4c3ec331811b7" : // dropdownBlock
				{
					$val = $this->ImPropArrData[$imitem['im_id']][$im_prop_id]['im_prop_value_dict_list'];
					if (! empty ( $val )) {
						$val = explode ( " ", $val );
						foreach ( $val as $lkey => $lvalue ) {
							if (! empty ( $lvalue ))
								$ret .= sprintf ( "%s, ", $this->DictData->getItemValue ( $lvalue ) );
						}
		...
		return $ret;
		/*
		 * 4c3ec1f67fe1b Выпадающий список 4c3ec1f67fe11 checkbox 4c3ec1f67fe12 radio 4c3ec331811b6 runner 4c3ec331811b7 dropdownBlock 4c3ec3ad35af9 text
		 */
	}
	public function urlDictToParent($dict_id, $i = 3) {
		//print_r($this->DictData);
		$value = $this->DictData->buld_table[$dict_id];
		$ret = "";
		while ( ! empty ( $value["parent_id"] ) ) {
			$ret = sprintf ( "%s_%s=1&", $value["dict_id"], ($value["dict_id"] == "4c3eb839f144e" ? 2 : $i) ) . $ret;
			$value = $this->DictData->buld_table[$value["parent_id"]];
			$i --;
		}
		$ret = sprintf ( "%s_%s=1&", $value["dict_id"], 0 ) . $ret;
		if ($ret)
