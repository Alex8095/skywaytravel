			...
			case '4c3ec1f67fe1b' :
				{
					...
					if ($Action == 'GetPropTextTr') {
						$return = "<tr class='i-{$PropData[im_prop_id]}'><td class='name'>{$PropData[im_prop_name]}</td><td class='val'>{$this->GetDictName($PropData[im_prop_value_dict_id])}</td></tr>";
					}
					if ($Action == 'GetTextWord') {
						$return = "{$PropData[im_prop_name]} - {$this->GetDictName($PropData[im_prop_value_dict_id])}<br>";
					}
					if ($Action == 'dl') {
						$return = "<dt>{$PropData[im_prop_name]}</dt><dd>{$this->GetDictName($PropData[im_prop_value_dict_id])}</dd>";
					}
					break;
				}
			#Флаг
			case '4c3ec1f67fe11' :
				{
					...
					if ($Action == 'GetPropTextTr') {
						$return = "<tr class='i-{$PropData[im_prop_id]}'><td class='name'>{$PropData[im_prop_name]}</td><td class='val'>+</td></tr>";
					}
					if ($Action == 'GetTextWord') {
						$return = "{$PropData[im_prop_name]} - есть<br>";
					}
					if ($Action == 'dl') {
						$return = "<dt>{$PropData[im_prop_name]}</dt><dd>+</dd>";
					}
					break;
				}
			#Радио-кнопки
			...
			case '4c3ec331811b6' :
				{
					...
					if ($Action == 'GetPropTextTr') {
						$return = "<tr class='i-{$PropData[im_prop_id]}'><td class='name'>{$PropData[im_prop_name]}</td><td class='val'>{$PropData['im_prop_value']}</td></tr>";
					}
					if ($Action == 'GetTextWord') {
						$return = "{$PropData[im_prop_name]} - {$PropData['im_prop_value']}<br>";
					}
					if ($Action == 'dl') {
						$return = "<dt>{$PropData[im_prop_name]}</dt><dd>{$PropData['im_prop_value']}</dd>";
					}
					break;
				}
			#Выпадающий блок
			case '4c3ec331811b7' :
				{
					...
					if ($Action == 'GetPropTextTr') {
						$PropValue = $this->StringToArray ( $PropData, 'GetText' );
						$return = "<tr class='i-{$PropData[im_prop_id]}'><td class='name'>{$PropData[im_prop_name]}</td><td class='val'>{$PropValue}</td></tr>";
					}
					if ($Action == 'GetTextWord') {
						$PropValue = $this->StringToArray ( $PropData, 'GetText' );
					...
					if ($Action == 'dl') {
						$PropValue = $this->StringToArray ( $PropData, 'GetText' );
						$return = "<dt>{$PropData[im_prop_name]}</dt><dd>{$PropValue}</dd>";
					}
					break;
				}
			#Поле ввода
