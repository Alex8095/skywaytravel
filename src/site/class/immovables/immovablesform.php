<?php
/*
 * Класс обработчик характеристик недвижимости @version class.catalog.php,v 1.0 2010/07/26 @author <AlexTsurkin/> @license GNU GPLv3
 */
class ImSiteForm {
	public $PostGet; // OST, GET
	public $FNameArray; // мя COOKIE поля для записи POST, GET, массивов
	public $FNameCode; // мя COOKIE поля для записи кода поля
	public $Query; // формулированный запрос для БД
	public $Dict;
	public $PropArrData;
	protected $RegDict;
	...
	/*
	 * Функция: @param $arr массив который надо расспарсить @param $pre_key айди поля принадлежности @return $string массива строка
	 */
	...
	private function getMergeExchangeRate($code) {
		...
		if ($code == "USD")
			return 1;
		if ($code == "UAH")
			return 1 / $exchangeRateObj->listBuild["USD"]["exchange"];
		if ($code == "EUR")
			return ($exchangeRateObj->listBuild["EUR"]["exchange"] / $exchangeRateObj->listBuild["USD"]["exchange"]);
		if ($code == "RUB")
			return 1 / ($exchangeRateObj->listBuild["USD"]["exchange"] /$exchangeRateObj->listBuild["RUB"]["exchange"]);
	}
	...
	public function PostGetParser($Arr) {
		...
		if (empty ( $Arr ))
			return;
		$exchange_rate_val = $this->getMergeExchangeRate ( $Arr["exchange_select"] );
							...
							case 'im_adress_id' :
								{
									...
									foreach ( $adressArray as $key => $value ) {
										...
										if (! empty ( $value )) {
											...
											if ($DictDataSelect)
												$adressQuery .= "'$DictDataSelect[dict_id]',";
											else {
									...
									$ImQuery .= (empty ( $adressQuery ) ? " AND i.im_date_add = '0' " : " and i.im_adress_id in (" . substr ( $adressQuery, 0, strlen ( $adressQuery ) - 1 ) . ")");
									break;
								}
							default :
								$ImQuery .= " AND i.$key = '$value'";
								break;
						}
					} else {
						$s_ = strpos ( $key, '_' );
		...
		$PropSelectQuery = "";
// 		devLogs::_printr($Arr["SearchIsAdvasedChecked"], "SearchIsAdvasedChecked");
...
// 			devLogs::_printr($PropSelectQuery, "PropSelectQuery");
			if ($PropSelectQuery)
				$PropSelectQuery = ' AND i.im_id IN ' . $PropSelectQuery;
			else
				return " AND i.im_id IN ('0')";
		}
// 		echo  $PropSelectQuery . $ImQuery . $TerSelectQuery;
	...
	/*
	 * Функция: формирует позиции для запроса SQl при select mylt @param @return
	 */
	...
	/*
	 * Функция: формирует рендомную букву для запроса БД @param $ArrIssetChar - массив использованных букв @return $ret - буква которая возвращаеться
	 */
