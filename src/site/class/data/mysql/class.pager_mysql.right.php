...
class pager_mysql_right extends pager_right {
	...
	public function get_total($left_join = null) {
		try {
	...
	public function get_page($myQuery = NULL, $leftJoin = null) {
		global $routingObj;
		// Текущая страница
		...
		$this->total = $total = $this->get_total ( $leftJoin );
		...
		if (! empty ( $myQuery ))
			$query = "{$myQuery} LIMIT $first, {$this->get_pnumber()}";
		else
